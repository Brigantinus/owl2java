...
public class XsdGraph {
	...
	private static final Graph<String, DefaultEdge> xsdGraph = new DefaultDirectedGraph<String, DefaultEdge>(
			DefaultEdge.class);
	...
	public static GraphPath<String, DefaultEdge> getShortestPath(String from, String to) {
		return DijkstraShortestPath.findPathBetween(xsdGraph, from, to);
	}
	...
	public static String findBestSuperXsdType(List<String> xsdTypeUris) {
		...
		for (String xsdTypeUri : xsdTypeUris) {
			...
			GraphPath<String, DefaultEdge> graphPath = graphUtils.findShortestPathDijkstra(XsdSchema.xsdLiteral, xsdTypeUri);
			paths.add(graphPath);
		}
